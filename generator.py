...
class Generator(object):
    ...
    def generate(self, n, min_v, max_v, n_uniques, n_swaps, rnd_seed=None):
        ...
        unique_vals = sample(set(self.sorted_l), abs(n_uniques))
        if n_uniques < 0:
            print('\nUNIQUE VALUES\n\t' + str(unique_vals), end='\n\n')
        self.few_unique_l = [choice(unique_vals) for i in range(0, n)]
        
        if n_swaps < 0:
            verbose = True
        self.nearly_l = Generator.nearly_sorted(self.sorted_l, abs(n_swaps), (n_swaps < 0))

    def nearly_sorted(sorted_l, n_swaps, verbose=False):
